...
type
  ...
  private
    ...
    //fListingDetails : TListingDetails;
    procedure ViewListingClick(Sender: TObject);


  public
    constructor Create(AOwner: TWinControl; APropertyID: Integer); reintroduce;
    procedure ViewListingDetails(Sender: TObject);
  ...
  private
    ...
    FCards: TList;
    fVisiblePanelCount : integer;
    fVisiblePanelLimit : integer;
    fListingDetailsContainer : TWinControl;
  ...
  public
    ...
    function Filter(Field, Value : string): TStringList; overload;
    function Filter(Wildcard : string): TStringList; overload;
    property CardCount : integer read fVisiblePanelCount write fVisiblePanelCount;
    property CardLimit : integer read fVisiblePanelLimit write fVisiblePanelLimit;
  end;
...
procedure TPropertyCard.ViewListingClick(Sender: TObject);
var
  i : integer;
begin
  //fListingDetails := TListingDetails.Create(fPropertyID);
  //fListingDetails.DisplayListingDetails(FListingDetailsContainer);
end;

procedure TPropertyCard.ViewListingDetails(Sender: TObject);
begin
  //
end;
...
constructor TPropertyCardManager.Create(AScrollBox: TScrollBox; AListingDetailsContainer: TWinControl);
begin
  inherited Create;
  FScrollBox := AScrollBox;
  fListingDetailsContainer := AListingDetailsContainer;
  FCards := TList.Create;
  fVisiblePanelCount := 0;
  fVisiblePanelLimit := 10;
end;
...
function TPropertyCardManager.Filter(Field, Value: string): TStringList;
var
  i: integer;
...
begin
  ...
  begin
    ...
    // Populate the TStringList with property ID's
...
function TPropertyCardManager.Filter(Wildcard: string): TStringList; // Continue here
var
  i: integer;
  lstPropertyID : TStringList;
begin
  ...
  begin
    ...
    qryProperties.SQL.Add('SELECT * FROM tblProperties WHERE Type = "Listing" ORDER BY PropertyID ASC');
    qryProperties.Open;
...
begin
  ...
  Card.OnClick := Card.ViewListingClick;
end;
